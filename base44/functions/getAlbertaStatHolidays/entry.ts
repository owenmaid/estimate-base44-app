...
const albertaHolidaysForYear = (year: number) => {
  ...
  return [
    { date: toIsoDate(utcDate(year, 0, 1)), name: "New Year's Day", provinces: PROVINCE },
    { date: toIsoDate(nthWeekday(year, 1, 1, 3)), name: 'Alberta Family Day', provinces: PROVINCE },
    { date: toIsoDate(goodFriday), name: 'Good Friday', provinces: PROVINCE },
    { date: toIsoDate(mondayBefore(year, 4, 24)), name: 'Victoria Day', provinces: PROVINCE },
    { date: toIsoDate(utcDate(year, 6, 1)), name: 'Canada Day', provinces: PROVINCE },
    { date: toIsoDate(nthWeekday(year, 8, 1, 1)), name: 'Labour Day', provinces: PROVINCE },
    { date: toIsoDate(nthWeekday(year, 9, 1, 2)), name: 'Thanksgiving Day', provinces: PROVINCE },
    { date: toIsoDate(utcDate(year, 10, 11)), name: 'Remembrance Day', provinces: PROVINCE },
    { date: toIsoDate(utcDate(year, 11, 25)), name: 'Christmas Day', provinces: PROVINCE },
  ];
};
...
Deno.serve(async (req) => {
  try {
    ...
    const { startDate, endDate, province = PROVINCE } = await req.json();
    ...
    if (String(province).toUpperCase() !== PROVINCE) {
