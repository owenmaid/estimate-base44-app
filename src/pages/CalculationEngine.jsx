...
function EquipmentRowCells({ row, inventoryValueMap, rowSums, rowHours, rowCol3, rowCol4, rowCol5, rowCol6, rowCol7, rowCol8, gridData, setGridData }) {
  ...
  const activeCell = (val, title) => (
    <div className={activeStyle} title={title}>
      {val !== null && val !== undefined && val !== 0 ? (typeof val === 'number' && !Number.isInteger(val) ? val.toFixed(2) : val) : '—'}
    </div>
  );
  const inactiveCell = <div className={inactiveStyle}>—</div>;

  return (
    <>
      {/* Shift Hrs */}
      <td className="px-1 py-1 border-r border-border">
        {isManpower ? activeCell(shiftHrs) : inactiveCell}
      </td>
      ...
      {COL_HEADERS.map((_, colIdx) => {
        ...
        let cellContent;
        if (isInactive) {
          cellContent = inactiveCell;
        } else if (isCol1) {
          cellContent = activeCell(col1Value || null);
        } else if (isCol2) {
          cellContent = activeCell(col2Value || null, `${col1Value} × ${shiftHrs}h`);
        } else if (isCol3) {
          cellContent = activeCell(col3Value || null, 'Col4+Col5+Col6+Col7+Col8');
        } else if (isCol4) {
          cellContent = activeCell(col4Value || null, '(N×8)+(Sa×4)');
        } else if (isCol5) {
          cellContent = activeCell(col5Value || null, `N×${shiftHrs - 8} OT hrs`);
        } else if (isCol6) {
          cellContent = activeCell(col6Value || null, `Sa×${Math.max(shiftHrs - 4, 0)}`);
        } else if (isCol7) {
          cellContent = activeCell(col7Value || null, `Su×${shiftHrs}`);
        } else if (isCol8) {
          cellContent = activeCell(col8Value || null, `St×${shiftHrs}`);
        } else if (isCol9) {
          cellContent = activeCell(col9Value, 'Reg Rate');
        } else if (isCol10) {
...
export default function CalculationEngine() {
  ...
  const handleDelete = (draft) => { if (draft.id) deleteMutation.mutate(draft.id); };
  const handleToggle = (draft) => { if (!draft.id) return; updateMutation.mutate({ id: draft.id, data: { ...draft, is_active: !draft.is_active } }); };
    ...
    <div className="p-6 space-y-6">
                ...
                {equipmentRows.length === 0 ? (
                  ...
                  equipmentRows.map((row) => (
                    <tr key={row.id} className="border-b border-border hover:bg-secondary/20 transition-colors" style={{ height: '40px' }}>
                      ...
                      <EquipmentRowCells
                        row={row}
                        inventoryValueMap={inventoryValueMap}
                        rowSums={rowSums}
                        rowHours={rowHours}
                        rowCol3={rowCol3}
                        rowCol4={rowCol4}
      ...
      {showNew && (
        <div>
          ...
          <FormulaEditor formula={blankFormula()} onSave={handleSave} onDelete={() => setShowNew(false)} isNew />
        </div>
      )}
