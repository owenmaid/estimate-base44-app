    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-6">
      ...
      {(() => {
        ...
        const kpiEquipment = isFiltered
          ? selectedCosts.rowCosts.filter(r => r.item_group === 'Equipment Group').reduce((s, r) => s + r.total, 0)
          : allProjectsSummary.reduce((s, p) => s + p.costs.rowCosts.filter(r => r.item_group === 'Equipment Group').reduce((a, r) => a + r.total, 0), 0);
        ...
        return (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className={isFiltered ? 'border-primary/30' : ''}>
              <CardContent className="p-5">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">
                    {isFiltered ? 'Project Status' : 'Total Projects'}
                  </span>
                ...
                {isFiltered ? (
                  <>
                    <div className="text-2xl font-bold capitalize">{STATUS_LABELS[selectedProject.status] || selectedProject.status}</div>
                    <div className="text-xs text-muted-foreground mt-1">{selectedProject.client || 'No client'}</div>
                  </>
                ) : (
                  <>
                    <div className="text-2xl font-bold">{projects.length}</div>
                    <div className="text-xs text-muted-foreground mt-1">{projects.filter(p => p.status === 'active').length} active</div>
                  </>
                )}
              </CardContent>
            </Card>
