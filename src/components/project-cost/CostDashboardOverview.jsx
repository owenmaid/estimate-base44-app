    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 sm:gap-4">
      {isFiltered && (
        <div className="col-span-2 lg:col-span-5 flex items-center gap-2 text-xs text-primary bg-primary/8 border border-primary/20 rounded-lg px-3 py-2">
          ...
          <span className="truncate">Showing values for: <span className="font-semibold">{selectedProject.name}</span></span>
          {selectedProject.project_number && <span className="text-muted-foreground hidden sm:inline">#{selectedProject.project_number}</span>}
        </div>
      )}
        ...
        <Card key={key} className={isFiltered ? selectedBorder : ''}>
          <CardContent className="p-3 sm:p-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className={`h-7 w-7 sm:h-8 sm:w-8 rounded-lg ${iconBox} flex items-center justify-center`}>
                <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${iconColor}`} />
              </div>
            </div>
            <div className="text-base sm:text-xl font-bold tabular-nums">{money(values[key])}</div>
